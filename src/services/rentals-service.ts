...
async function checkMoviesValidForRental(moviesId: number[], user: User) {
  for (let i = 0; i < moviesId.length; i++) {
    ...
    if (movieAlreadyRented(movie.rentalId)) {
      throw movieAlreadyInRental("Movie already in a rental.");
    }

    if(userUnderAgeWithAdultMovie(movie.adultsOnly, user)){
...
function userUnderAgeWithAdultMovie(adultsOnly: boolean, user: User){
  return adultsOnly && userIsUnderAge(user) 
}
